...
# @router.get("quiz/{quiz_id}/question", response_model=QuestionPublic)
# def get_question_api(session: SessionDep, quiz_id:int, teacher: CurrentUserDep, ques_id=Annotated[Union[int, None], Query()]):
#     if not get_quiz_owner_id(session=session, quiz_id=quiz_id, teacher_id=teacher.id):
#         raise HTTPException(
#                 status_code=400, detail="Do not have permissions")
