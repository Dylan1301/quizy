...
from models.quiz import Quiz, QuizCreate, QuizPublic, QuizQuestions, QuizzesPublic
from core.db.db import authenticate_teacher, get_teacher_by_email, get_password_hash, create_teacher
from core.db.quiz import get_quizzes, create_quiz, get_quiz
...
def get_all_quiz(session: SessionDep, teacher: CurrentUserDep, skip: int = 0, limit: int = 100) -> Any:
    response = get_quizzes(session=session, teacher_id=teacher.id, skip=skip, limit=limit)
    return response
